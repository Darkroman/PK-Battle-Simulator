...
Menu::Menu(std::vector<Player>& players)
{
	Menu::players[0] = &(players[0]);
	Menu::players[1] = &(players[1]);
...
void Menu::ChangePlayerOneType()
{
	...
	while(exit == false)
	{
		...
		case 2:
			players[0]->SwitchTypeToAI();
			std::cout << "Switched Player One to A.I.\n\n";
			break;
	...
	while (exit == false)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					std::cout << "Invalid input! Please enter a number between 1 and 164, or valid name of a move.\n";
				}
				else
				{
					if (!player->GetBelt(beltslot)->HasMove(moveslot))
