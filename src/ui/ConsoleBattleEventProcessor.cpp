...
void ConsoleBattleEventProcessor::ProcessEvents()
{
    ...
    {
        QueuedBattleEvent queuedBattleEvent = m_eventQueue.TakeNextEvent();

        std::string formattedText{};

        switch (queuedBattleEvent.event)
        {
            // --- NO ARGUMENTS ---
        ...
        {
            formattedText = BattleText::GetFormat(queuedBattleEvent.event);
            break;
        }
        ...
        {
            auto fmtStr = BattleText::GetFormat(queuedBattleEvent.event);
            formattedText = std::vformat(fmtStr,
                std::make_format_args(std::get<unsigned int>(queuedBattleEvent.args[0])));
            break;
        }
        ...
        case BattleEvents::DigCharge:
        {
            auto fmtStr = BattleText::GetFormat(queuedBattleEvent.event);
            formattedText = std::vformat(fmtStr,
                std::make_format_args(std::get<std::string_view>(queuedBattleEvent.args[0])));
            break;
        }
        ...
        case BattleEvents::FieldEffectFaded:
        {
            ...
            formattedText = std::vformat(fmtStr,
                std::make_format_args(std::get<std::string_view>(queuedBattleEvent.args[0]),
                    std::get<std::string_view>(queuedBattleEvent.args[1])
                ));
            break;
        }
        ...
        case BattleEvents::MultiHit:
        {
            ...
            formattedText = std::vformat(fmtStr,
                std::make_format_args(std::get<std::string_view>(queuedBattleEvent.args[0]),
                    std::get<int>(queuedBattleEvent.args[1])
                ));
            break;
        }
        ...
        case BattleEvents::MoveNoLongerDisabled:
        {
            ...
            formattedText = std::vformat(fmtStr,
                std::make_format_args(std::get<std::string_view>(queuedBattleEvent.args[0]),
                    std::get<std::string_view>(queuedBattleEvent.args[1]),
                    std::get<std::string_view>(queuedBattleEvent.args[2])
                ));
            break;
        }
        ...
        case BattleEvents::Wrap:
        {
            ...
            formattedText = std::vformat(fmtStr,
                std::make_format_args(std::get<std::string_view>(queuedBattleEvent.args[0]),
                    std::get<std::string_view>(queuedBattleEvent.args[1]),
                    std::get<std::string_view>(queuedBattleEvent.args[2]),
                    std::get<std::string_view>(queuedBattleEvent.args[3])
                ));
            break;
        }
        ...
        m_outputTarget.Emit(std::move(formattedText));
    }
}
